...
function narrowAllSquaresByRowAndColumn(board: square[][]) {
    ...
    for (let col = 0; col < board[row].length; col++) {
      ...
      possibleValues.forEach((pValue) => {
        ...
        if (deducible) {
          ...
          regions[board[row][col].region].delete(pValue);
          return;
        }
      });
